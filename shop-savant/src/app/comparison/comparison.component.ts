...
export class ComparisonComponent {
  ...
  ngOnInit(): void {
    ...
    // for (let i =0; i<3;i++){
    //   Object.keys(this.hardcoded_input[0].attributeTable).forEach(function(key) {
    //     console.log(key, this.hardcoded_input[0].attributeTable)
    //     // this.temp[0] = [key, this.hardcoded_input[0].attributeTable]}
    //   }
    // }

    for(let product of this.hardcoded_input) {

      this.commonAttributes = this.commonAttributes.concat(Object.keys(product.attributeTable))
      this.commonAttributes = this.commonAttributes.filter((item, pos) => this.commonAttributes.indexOf(item)===pos);
    }
    console.log(this.commonAttributes)
  }
}
